 ...
 * Ref: https://zero4drift.github.io/posts/csapp-shlab-jie-ti-si-lu-ji-lu/#eval
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
{
    pid_t pid = 0;
    int state, bg_or_not, buildin_or_not, is_avaiable_cmd;
    char *args[MAXARGS];
    char *first_cmd;
    sigset_t blockset;
    const char *err_msg = "%s: Command not found\n";
    sigemptyset(&blockset);        // create empty signal set.
    sigaddset(&blockset, SIGCHLD); // add SIGCHILD signal
    bg_or_not = args[0];
    buildin_or_not = builtin_cmd(args);

    if (!buildin_or_not)
    {
        is_avaiable_cmd = access(first_cmd, 00);
        if (is_avaiable_cmd == -1)
        {
        ...
        if (pid == 0)
        {
            // 子进程: 修改进程组号码
            setpgid(0, 0);
            // 这里需要注意，子进程fork的时候同时保留了对sigchld信号的阻塞，需要移除阻塞
            sigprocmask(SIG_UNBLOCK, &blockset, NULL);
